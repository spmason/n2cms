...
namespace N2.Plugin
{
	...
	public class PluginBootstrapper : IPluginBootstrapper
	{
		...
		#region IPluginBootstrapper Members
			...
			foreach (PluginInitializerElement configElement in addedInitializers)
			{
				...
				if (typeof (IPluginDefinition).IsAssignableFrom(pluginType))
					throw new N2Exception(
						"The configured plugin initializer type '{0}' is not a valid plugin initializer since it doesn't implement the IPluginDefinition interface.",
						configElement.Type);
		...
		public void InitializePlugins(IEngine engine, IEnumerable<IPluginDefinition> plugins)
		{
			...
			{
				string message =
					"While initializing {0} plugin(s) threw an exception. Please review the stack trace to find out what went wrong.";
